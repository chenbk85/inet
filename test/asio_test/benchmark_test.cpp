...
class reactor_connect_benchmark : public ::testing::TestWithParam<boost::tuple<int, int>> 
{
};

TEST_P(reactor_connect_benchmark, connect_test)
{
	int time_out, connect_count;
	boost::tie(time_out, connect_count) = GetParam();
	tbb::atomic<int> accepted_count;
	tbb::atomic<int> accept_failed_count;
	tbb::atomic<int> connected_count;
	tbb::atomic<int> connect_failed_count;
...
class reactor_chat_benchmark : public ::testing::TestWithParam<int> 
{
};

TEST_P(reactor_chat_benchmark, chat_test)
{
	int connect_count = GetParam();
...
// <0> thread count [0 = default] <1> connect count
INSTANTIATE_TEST_CASE_P(benchmark, multi_thread_chat_benchmark, ::testing::Values(
	boost::tuples::make_tuple(0, 50)
	));

// <0> time(milliseconds) <1> connect count
INSTANTIATE_TEST_CASE_P(benchmark, reactor_connect_benchmark, ::testing::Values(
	boost::tuples::make_tuple(500, 100)
	));

// connect count
INSTANTIATE_TEST_CASE_P(benchmark, reactor_chat_benchmark, ::testing::Values(
	50
	));
...
INSTANTIATE_TEST_CASE_P(benchmark, reactor_connect_benchmark, ::testing::Values(
	boost::tuples::make_tuple(500, 100),
	boost::tuples::make_tuple(500, 300),
	boost::tuples::make_tuple(500, 500),
	boost::tuples::make_tuple(999, 500),
	boost::tuples::make_tuple(999, 999)
	));
